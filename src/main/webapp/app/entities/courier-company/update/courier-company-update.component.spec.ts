jest.mock('@angular/router');

import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { FormBuilder } from '@angular/forms';
import { ActivatedRoute } from '@angular/router';
import { of, Subject } from 'rxjs';

import { CourierCompanyService } from '../service/courier-company.service';
import { ICourierCompany, CourierCompany } from '../courier-company.model';

import { CourierCompanyUpdateComponent } from './courier-company-update.component';

describe('Component Tests', () => {
  describe('CourierCompany Management Update Component', () => {
    let comp: CourierCompanyUpdateComponent;
    let fixture: ComponentFixture<CourierCompanyUpdateComponent>;
    let activatedRoute: ActivatedRoute;
    let courierCompanyService: CourierCompanyService;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        declarations: [CourierCompanyUpdateComponent],
        providers: [FormBuilder, ActivatedRoute],
      })
        .overrideTemplate(CourierCompanyUpdateComponent, '')
        .compileComponents();

      fixture = TestBed.createComponent(CourierCompanyUpdateComponent);
      activatedRoute = TestBed.inject(ActivatedRoute);
      courierCompanyService = TestBed.inject(CourierCompanyService);

      comp = fixture.componentInstance;
    });

    describe('ngOnInit', () => {
      it('Should update editForm', () => {
        const courierCompany: ICourierCompany = { id: 456 };

        activatedRoute.data = of({ courierCompany });
        comp.ngOnInit();

        expect(comp.editForm.value).toEqual(expect.objectContaining(courierCompany));
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', () => {
        // GIVEN
        const saveSubject = new Subject();
        const courierCompany = { id: 123 };
        spyOn(courierCompanyService, 'update').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ courierCompany });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.next(new HttpResponse({ body: courierCompany }));
        saveSubject.complete();

        // THEN
        expect(comp.previousState).toHaveBeenCalled();
        expect(courierCompanyService.update).toHaveBeenCalledWith(courierCompany);
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', () => {
        // GIVEN
        const saveSubject = new Subject();
        const courierCompany = new CourierCompany();
        spyOn(courierCompanyService, 'create').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ courierCompany });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.next(new HttpResponse({ body: courierCompany }));
        saveSubject.complete();

        // THEN
        expect(courierCompanyService.create).toHaveBeenCalledWith(courierCompany);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).toHaveBeenCalled();
      });

      it('Should set isSaving to false on error', () => {
        // GIVEN
        const saveSubject = new Subject();
        const courierCompany = { id: 123 };
        spyOn(courierCompanyService, 'update').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ courierCompany });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.error('This is an error!');

        // THEN
        expect(courierCompanyService.update).toHaveBeenCalledWith(courierCompany);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).not.toHaveBeenCalled();
      });
    });
  });
});
