...
export class CourierCompanyComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.courierCompanyService
      ...
      .subscribe(
        (res: HttpResponse<ICourierCompany[]>) => {
  ...
  protected onSuccess(data: ICourierCompany[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/courier-company'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
